...
InternetProtocolProvider* InternetProtocolProvider::ActiveInstance = 0;
...
void InternetProtocolProvider::Send(uint32_t dstIP_BE, uint8_t protocol, uint8_t *data, uint32_t size)
{
    ...
    uint8_t *buffer = (uint8_t *)MemoryManager::ActiveMemoryManager->MemAllocate(sizeof(InternetProtocolV4Message) + size);
